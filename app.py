...
# OJO: esta app usa memoria local. Para Railway conviene 1 solo worker.
estado = {
    "corriendo": False,
    "progreso": [],
...
async def login(page, usuario, password, login_url):
    ...
    await page.wait_for_url("**/Index.aspx", timeout=20000)
...
async def navegar_certificado_tope(page):
    ...
    try:
        ...
        if await aceptar.count() > 0 and await aceptar.is_visible(timeout=3000):
            await aceptar.click()
            await page.wait_for_timeout(800)
    except Exception:
        pass
...
async def get_frame(page):
    for frame in page.frames:
        if "CertificadoTope" in frame.url:
            return frame
...
async def buscar(frame):
    await frame.evaluate("""
        (() => {
            const btn = document.querySelector('#btSearch');
            if (btn) btn.click();
        })();
    """)
...
async def descargar_pdf(page, frame, idx: int) -> dict:
    ...
    try:
        async with page.expect_response(
            lambda r: "CrearCertificado" in r.url,
            timeout=15000,
        ) as resp_info:
            await frame.evaluate("""
                (() => {
    ...
    async with async_playwright() as p:
        try:
            ...
            for idx, row in enumerate(registros, 1):
                ...
                log(f"[{idx}/{len(registros)}] Buscando {tipo_doc} {numero} — {tipo_amparo}")
        ...
        for rn in range(2, len(df) + 2):
            ...
            if col_estado:
                ...
                if "DESCARGADO" in val:
                    ce.fill = verde
                    ce.font = Font(bold=True, color="FFFFFF", size=10)
...
def iniciar():
    ...
    try:
        ...
        if missing:
            return jsonify({"error": f"Faltan columnas: {', '.join(sorted(missing))}"}), 400

        df = df.dropna(subset=["ID"])
        df["TIPO"] = df["TIPO"].astype(str).str.strip().str.upper()
        df["ID"] = df["ID"].astype(str).str.strip()
        df["TIPO DE AMPARO"] = df["TIPO DE AMPARO"].astype(str).str.strip().str.upper()
